    ...
    <div className="w-screen h-screen overflow-hidden bg-black relative font-sans text-white">
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .country-wrapper { 
            ...
            cursor: pointer; 
            pointer-events: auto;
            will-change: transform; /* 🟢 PERFORMANCE FIX: Forces Hardware Acceleration */
        }
        ...
        @keyframes radar-ping {
            0% { box-shadow: 0 0 0 0 rgba(0, 246, 255, 0.8); }
            70% { box-shadow: 0 0 0 20px rgba(0, 246, 255, 0); }
            100% { box-shadow: 0 0 0 0 rgba(0, 246, 255, 0); }
        }
        .country-dot.target-ping {
            width: 8px;
            height: 8px;
            background-color: #00f6ff;
            box-shadow: 0 0 15px 5px rgba(0, 246, 255, 0.9);
            animation: radar-ping 2s infinite cubic-bezier(0.66, 0, 0, 1);
        }
        ...
        /* 🟢 PERFORMANCE FIX: Removed blur, added visibility toggle */
        .country-popover {
          position: absolute;
          bottom: 24px;
          ...
          gap: 10px;
          opacity: 0;
          visibility: hidden; /* Tells the GPU to stop calculating this until needed */
        ...
        .country-wrapper:hover .country-popover { 
            opacity: 1; 
            visibility: visible;
            transform: translateY(0) scale(1); 
        }
        
        .country-wrapper:hover .country-label { 
            color: #3b82f6; 
            transform: scale(1.2); 
            text-shadow: 0 0 10px rgba(59,130,246,0.5);
        }
        .country-wrapper:hover .country-dot:not(.target-ping) {
            transform: scale(1.5);
            background-color: #60a5fa;
            box-shadow: 0 0 12px 4px rgba(59, 130, 246, 0.9);
        }
      ...
      <div className="fixed top-8 left-8 z-[100] pointer-events-none select-none hidden md:block">
        <h1 className="text-xl md:text-2xl font-black tracking-tighter m-0 p-0 leading-none">
            EARTH<span className="text-blue-500">LOOKUP</span><span className="text-[10px] text-gray-500 ml-1">.COM</span>
        </h1>
        <h2 className="text-[8px] font-bold tracking-[0.2em] text-gray-400 uppercase mt-1">Ultimate Geography Encyclopedia</h2>
      </div>

      {/* BOTTOM-LEFT: GLOBAL COMPLIANCE FOOTER (AdSense Ready) */}
      <div className="fixed bottom-8 left-4 md:left-8 z-[100] flex flex-col gap-3 bg-black/50 border border-white/10 p-5 rounded-xl backdrop-blur-md shadow-2xl pointer-events-auto">
        ...
        <div className="text-[8px] md:text-[9px] font-mono text-gray-600 font-bold tracking-widest uppercase">
          © 2026 // EarthLookup Systems
        </div>
      </div>
      ...
      <div className="fixed top-8 right-4 md:right-8 z-[100] w-[calc(100%-2rem)] md:w-full md:max-w-[280px]">
        <div className="bg-black/60 border border-white/10 rounded-full flex items-center px-4 backdrop-blur-md shadow-lg pointer-events-auto">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-gray-500 min-w-[16px]">
            <circle cx="11" cy="11" r="8"></circle>
            <line x1="21" y1="21" x2="16.65" y2="16.65"></line>
          </svg>
          <input
            type="text"
            className="w-full bg-transparent border-none py-3 px-3 text-sm focus:outline-none placeholder:text-gray-500 font-medium tracking-wide"
            placeholder="Search a country..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            onFocus={() => setIsFocused(true)}
            onBlur={() => setTimeout(() => setIsFocused(false), 200)}
            onKeyDown={(e) => e.key === 'Enter' && filteredNations[0] && flyToTarget(filteredNations[0])}
          />
        </div>
            ...
            {filteredNations.slice(0, 10).map(n => (
              <button key={n.slug} onClick={() => flyToTarget(n)} className="w-full text-left px-4 py-3 hover:bg-blue-500/10 text-sm flex items-center gap-3 border-b border-white/5 last:border-0">
                ...
                <div className="flex flex-col">
                    ...
                    <span className="text-[9px] text-gray-500 uppercase tracking-tighter">{n.code}</span>
                </div>
              </button>
            ))}
          </div>
      ...
      <div className="fixed bottom-12 right-4 md:right-8 z-[100] flex flex-col bg-black/50 border border-white/10 rounded-lg backdrop-blur-md overflow-hidden shadow-2xl pointer-events-auto">
        <button 
          ...
          className="w-10 h-10 flex items-center justify-center text-white hover:bg-blue-500/20 hover:text-blue-400 transition-colors border-b border-white/10 text-xl font-light"
          aria-label="Zoom in"
        >
          +
        </button>
        <button 
          ...
          className="w-10 h-10 flex items-center justify-center text-white hover:bg-blue-500/20 hover:text-blue-400 transition-colors text-xl font-light"
          aria-label="Zoom out"
        >
          −
        </button>
      ...
      {/* THE HD VECTOR GLOBE WITH TARGET LOCK */}
        ...
        {dimensions.width > 0 && (
          <Globe
            ...
            polygonCapColor={(poly: any) => 
                selectedTarget && poly.properties.name === selectedTarget.name
                ? 'rgba(0, 246, 255, 0.3)' 
                : 'rgba(10, 10, 10, 0.6)'  
            }
            ...
            onPolygonClick={(poly: any) => {
                ...
                if (target) {
                    if (selectedTarget && selectedTarget.name === target.name) {
                        navigateToDossier(target.slug);
                    } else {
                        flyToTarget(target);
                    }
                }
              ...
              el.innerHTML = `
                ...
                <div class="country-popover ${isTarget ? 'force-open' : ''}">
                  <div class="popover-header">
                      <img src="${d.flag}" style="width: 22px; border-radius: 2px;" />
                  ...
                  <div class="popover-arrow ${isTarget ? 'animate-pulse text-[#00f6ff]' : ''}">
                      ${isTarget ? 'CLICK TO ACCESS' : 'ACCESS DOSSIER'} <span>→</span>
                  </div>
                </div>
